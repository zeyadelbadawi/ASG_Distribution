...
export default function PartnerForm({ partner, onSave, existingPartners }) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            {imageUrl && (
                <div>
                    <img src={imageUrl} alt="Partner" width="200px" />
                    <p>Current Image</p>
                </div>
            )}
                ...
                <ul>
                    {products.map((product) => (
                        <li key={product.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                            <span>{product.name} - {product.description} (Category: {product.category})</span>
                            {product.image && <img src={product.image} alt={product.name} width="100px" style={{ marginTop: '10px' }} />}
